...
util.confirmDoWithTitle = function (title, msg, fn) {
    layer.confirm(msg, {
        time: 0,
        title: title,
        btn: ['确定', '取消'],
...
util.getYDate = function (dateTimeStamp) {
...
util.thousand = function format(num) {
    var reg = /\d{1,3}(?=(\d{3})+$)/g
    num = num + ''
...
function checkhaslogin() {
    // if (util.confirmEnding(window.location.href, "xrx_portal/")) {
    // 	return;
    // }
    // $.ajax({
    // 	async: true,
    ...
    // 	timeout: 2000,
    // 	type: "POST",
    // 	url: rootpath + "/com/need/login",
    // 	error: function (jqXHR, textStatus, errorThrown) {
    // 		console.log(textStatus);
    // 		if (textStatus == "timeout") {
    // 			window.location.href = rootpath;
    // 		} else {
    // 			util.alert(textStatus);
    // 		}
    // 	},
    ...
    if (obj instanceof Array) {
        ...
        for (let item in obj) {
            ...
            if (val == null) {
                ...
                temp[item] = typeof val == 'object' ? util.clone(val) : val; //这里也没有判断是否为函数，因为对于函数，我们将它和一般值一样处理
            }
        }
    }
    return temp;
...
function hidecitysel() {
    $('.iscity .if').hide();
}

function ChooserClick(k) {
    var obj = k.target
    // if ($(obj).parents(".iscity").length > 0) {
    //     $('.iscity .if').show();
    // } else {
    //     if ($(obj).parents(".iin").length > 0) {
    //         showcitysel();
    //     } else {
    //         $('.iscity .if').hide();
    //     }
    ...
    if ($(obj).parent().hasClass('Chooser')) {
        var la = $(obj).parent().find('.la')
        if ($(la).parent().find('input').length > 0) {
            $(la).show()
        }
        return
