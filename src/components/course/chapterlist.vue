  ...
  <div>
    <div class="head-c">
      ...
      <div style="float:right;">
        <router-link tag="button" class="cb" :to="courseUrl">返 回</router-link>
      </div>
    </div>
    <div class="content">
      <div class="panel">
        ...
        <div v-loading="loading">
          <div class="result-c">
            <table class="comTb">
              ...
              <tr v-for="item in chapterlist" :key="item.id">
                <td>
                  <div class="ll">{{item.chapterName}}</div>
                </td>
                <td>
                  <div class="ll">{{item.description}}</div>
                </td>
                <td>
                  <router-link tag="span" class="operate" :to="item.sectionUrl">关联课节</router-link>
    ...
    <el-dialog title="编辑课章" :visible.sync="exerciseEditVisable" width="800px" center>
      <div class="panel">
        <div class="p-t p-t2">
          <div class="name">
            <span>编辑课章</span>
    ...
    <el-dialog title="提示" :visible.sync="delVisable" width="30%" center>
      <span>{{msg}}</span>
      <span style="margin-left:20px;color:#0dbc5c;">{{chapter.chapterName}}</span>
...
export default {
  ...
  methods: {
    getData(val) {
      ...
      that.loading = true;
      axios.post(api.api.course.chapterlist, param).then(response => {
        that.loading = false;
        var rdata = response.data;
        if (rdata.code == 0) {
          that.chapterlist = rdata.page.list;
          that.chapterlist.forEach(value => {
            value.sectionUrl =
    ...
    save() {
      var that = this;
      that.chapter.courseId = that.courseId;
      that.chapter.updateTime = null;
      var param = that.chapter;
      axios.post(api.api.chapter.save, param).then(response => {
        that.loading = false;
    ...
    delData() {
      ...
      axios.post(api.api.chapter.delete, param).then(response => {
