      ...
      <div class="panel">
        <div class="p-t p-t2">
          ...
          <div class="tools clearfix">
            <div class="t-r">
              <router-link tag="span" class="operate" to="/newsmsg/edit/0">添加资讯</router-link>
            </div>
          </div>
        </div>
        <div v-loading="loading">
          <div class="result-c">
            <table class="comTb">
              ...
              <tr v-for="item in newsMsgList" :key="item.id">
                <td>{{item.msgName}}</td>
                <td>
  ...
  methods: {
    ...
    getData(val) {
      ...
      axios.post(api.api.msg.list, param).then(response => {
        ...
        var rdata = response.data;
        if (rdata.code == 0) {
          that.newsMsgList = rdata.page.list;
          that.newsMsgList.forEach(item => {
            item.url = "/newsmsg/edit/" + item.id;
          });
