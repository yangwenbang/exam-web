    ...
    <div class="wrap">
      ...
      <div class="c">
        <div class="login-input">
          ...
          <div class="hint" v-if="isPhoneShow && (hint === '请输入手机号' || hint === '请输入正确的手机号') ">{{hint}}</div>
        </div>
        <div class="login-input">
        ...
        <div class="login-input">
          <input type="password" name="password1" style="display:none"/><!--禁用浏览器input自动填充-->
          <input type="password" v-model="registerData.upassword" placeholder="请输入密码" autocomplete="off" @focus="pswTip"
                 @blur="pswReg">
          <div class="hint" v-if="isPswShow">{{tip}}</div>
        </div>
        <div class="login-input">
          <input type="password" v-model="pswAgain" placeholder="请再次输入密码" autocomplete="off">
          <div class="hint" v-if=" isShowHint && (hint === '两次密码不一致,请重新输入' || hint === '请再次输入密码' ) ">{{hint}}</div>
        </div>
        <div class="login-input v clearfix">
          <div class="v-input">
            <input type="text" placeholder="请输入验证码" v-model="Vcode.validatecode">
          </div>
          <div class="v-img"><img :src="imgCode" alt="" @click="getImgCode"></div>
        </div>
        <div class="login-input">
          <button class="rgbtn" @click="registerbtn">注册</button>
          <div class="hint" v-if=" isShowHint"><span>{{msg}}</span></div>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      registerbtn () {
        ...
        axios.post(api.api.user.register, param).then(response => {
          if (response.data.bresult) {
      ...
      getImgCode () {
        this.imgCode = CONSTANT.imgCodePath + '/imgcode/getvalidatecode' + '?time=' + new Date()
      },
      checkLogin () {
        var that = this
